...
const Search: React.FunctionComponent<Props> = () => {
  ...
  const searchProductsAsync = useAsync(() => searchApi.searchProduct({ name: searchStr ?? "" }), false);
  const searchCraftsmenAsync = useAsync(() => searchApi.searchCraftsman({ job: searchStr ?? "" }), false);
  ...
  const suggestedProductsResults = useAsync(() => productApi.getProductList().then(res => _.take(res, 2)), false);
            ...
            <div id="result-list">
              {
                isProduct
                  ...
                  : filteredCraftsmen?.map(elem => <CraftsmanresultCard img={elem.profilePicture ?? "/img/craftsman/default.svg"} name={elem.firstname} job={elem.job ?? ""} href={`/app/craftsman/${elem?.id ?? ""}`} />)
              }
            </div>
          </section>
        </div>
