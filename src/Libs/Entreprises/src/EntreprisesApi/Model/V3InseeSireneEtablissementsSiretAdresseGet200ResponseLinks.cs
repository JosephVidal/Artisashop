/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks
    /// </summary>
    [DataContract(Name = "_v3_insee_sirene_etablissements__siret__adresse_get_200_response_links")]
    public partial class V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks : IEquatable<V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks" /> class.
        /// </summary>
        /// <param name="entreprise">Lien vers la ressource &#39;Donnée Sirene d&#39;une unité légale&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;unité légale. (required).</param>
        /// <param name="etablissement">Lien vers la ressource &#39;Donnée Sirene d&#39;un établissement&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;établissement. (required).</param>
        public V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks(string entreprise = default(string), string etablissement = default(string))
        {
            // to ensure "entreprise" is required (not null)
            if (entreprise == null)
            {
                throw new ArgumentNullException("entreprise is a required property for V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks and cannot be null");
            }
            this.Entreprise = entreprise;
            // to ensure "etablissement" is required (not null)
            if (etablissement == null)
            {
                throw new ArgumentNullException("etablissement is a required property for V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks and cannot be null");
            }
            this.Etablissement = etablissement;
        }

        /// <summary>
        /// Lien vers la ressource &#39;Donnée Sirene d&#39;une unité légale&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;unité légale.
        /// </summary>
        /// <value>Lien vers la ressource &#39;Donnée Sirene d&#39;une unité légale&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;unité légale.</value>
        [DataMember(Name = "entreprise", IsRequired = true, EmitDefaultValue = true)]
        public string Entreprise { get; set; }

        /// <summary>
        /// Lien vers la ressource &#39;Donnée Sirene d&#39;un établissement&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;établissement.
        /// </summary>
        /// <value>Lien vers la ressource &#39;Donnée Sirene d&#39;un établissement&#39; correspondant à cette adresse d&#39;établissement pour obtenir les données de référence de l&#39;établissement.</value>
        [DataMember(Name = "etablissement", IsRequired = true, EmitDefaultValue = true)]
        public string Etablissement { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks {\n");
            sb.Append("  Entreprise: ").Append(Entreprise).Append("\n");
            sb.Append("  Etablissement: ").Append(Etablissement).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks);
        }

        /// <summary>
        /// Returns true if V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks instances are equal
        /// </summary>
        /// <param name="input">Instance of V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(V3InseeSireneEtablissementsSiretAdresseGet200ResponseLinks input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.Entreprise == input.Entreprise ||
                    (this.Entreprise != null &&
                    this.Entreprise.Equals(input.Entreprise))
                ) && 
                (
                    this.Etablissement == input.Etablissement ||
                    (this.Etablissement != null &&
                    this.Etablissement.Equals(input.Etablissement))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.Entreprise != null)
                {
                    hashCode = (hashCode * 59) + this.Entreprise.GetHashCode();
                }
                if (this.Etablissement != null)
                {
                    hashCode = (hashCode * 59) + this.Etablissement.GetHashCode();
                }
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
