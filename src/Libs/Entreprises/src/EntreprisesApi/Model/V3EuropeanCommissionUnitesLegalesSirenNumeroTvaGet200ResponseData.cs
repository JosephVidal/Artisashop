/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData
    /// </summary>
    [DataContract(Name = "_v3_european_commission_unites_legales__siren__numero_tva_get_200_response_data")]
    public partial class V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData : IEquatable<V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData" /> class.
        /// </summary>
        /// <param name="tvaNumber">Ce numéro de TVA est extrait du numéro de SIREN, à l&#39;aide de la règle de calcul du numéro de TVA, et validé auprès du service VIES de la Commission européenne. Ce numéro de TVA est forcément un numéro de TVA français. (required).</param>
        public V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData(string tvaNumber = default(string))
        {
            // to ensure "tvaNumber" is required (not null)
            if (tvaNumber == null)
            {
                throw new ArgumentNullException("tvaNumber is a required property for V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData and cannot be null");
            }
            this.TvaNumber = tvaNumber;
        }

        /// <summary>
        /// Ce numéro de TVA est extrait du numéro de SIREN, à l&#39;aide de la règle de calcul du numéro de TVA, et validé auprès du service VIES de la Commission européenne. Ce numéro de TVA est forcément un numéro de TVA français.
        /// </summary>
        /// <value>Ce numéro de TVA est extrait du numéro de SIREN, à l&#39;aide de la règle de calcul du numéro de TVA, et validé auprès du service VIES de la Commission européenne. Ce numéro de TVA est forcément un numéro de TVA français.</value>
        [DataMember(Name = "tva_number", IsRequired = true, EmitDefaultValue = true)]
        public string TvaNumber { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData {\n");
            sb.Append("  TvaNumber: ").Append(TvaNumber).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData);
        }

        /// <summary>
        /// Returns true if V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData instances are equal
        /// </summary>
        /// <param name="input">Instance of V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(V3EuropeanCommissionUnitesLegalesSirenNumeroTvaGet200ResponseData input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.TvaNumber == input.TvaNumber ||
                    (this.TvaNumber != null &&
                    this.TvaNumber.Equals(input.TvaNumber))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.TvaNumber != null)
                {
                    hashCode = (hashCode * 59) + this.TvaNumber.GetHashCode();
                }
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
