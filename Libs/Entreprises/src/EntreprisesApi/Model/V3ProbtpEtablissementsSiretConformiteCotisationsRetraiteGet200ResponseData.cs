/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData
    /// </summary>
    [DataContract(Name = "_v3_probtp_etablissements__siret__conformite_cotisations_retraite_get_200_response_data")]
    public partial class V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData : IEquatable<V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData>, IValidatableObject
    {
        /// <summary>
        /// Détermine si une entreprise est éligible ou non aux cotisations retraites de ProBTP.
        /// </summary>
        /// <value>Détermine si une entreprise est éligible ou non aux cotisations retraites de ProBTP.</value>
        [JsonConverter(typeof(StringEnumConverter))]
        public enum EligibleEnum
        {
            /// <summary>
            /// Enum True for value: true
            /// </summary>
            True = 1,

            /// <summary>
            /// Enum False for value: false
            /// </summary>
            False = 0

        }


        /// <summary>
        /// Détermine si une entreprise est éligible ou non aux cotisations retraites de ProBTP.
        /// </summary>
        /// <value>Détermine si une entreprise est éligible ou non aux cotisations retraites de ProBTP.</value>
        [DataMember(Name = "eligible", IsRequired = true, EmitDefaultValue = true)]
        public EligibleEnum Eligible { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData" /> class.
        /// </summary>
        /// <param name="eligible">Détermine si une entreprise est éligible ou non aux cotisations retraites de ProBTP. (required).</param>
        public V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData(EligibleEnum eligible = default(EligibleEnum))
        {
            this.Eligible = eligible;
        }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData {\n");
            sb.Append("  Eligible: ").Append(Eligible).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData);
        }

        /// <summary>
        /// Returns true if V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData instances are equal
        /// </summary>
        /// <param name="input">Instance of V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(V3ProbtpEtablissementsSiretConformiteCotisationsRetraiteGet200ResponseData input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.Eligible == input.Eligible ||
                    this.Eligible.Equals(input.Eligible)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                hashCode = (hashCode * 59) + this.Eligible.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
