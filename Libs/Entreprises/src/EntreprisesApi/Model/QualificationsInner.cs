/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// QualificationsInner
    /// </summary>
    [DataContract(Name = "Qualifications_inner")]
    public partial class QualificationsInner : IEquatable<QualificationsInner>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="QualificationsInner" /> class.
        /// </summary>
        /// <param name="nom">Nom de la qualification.</param>
        /// <param name="codeQualification">Code de la qualification.</param>
        /// <param name="definition">Définition de la qualification.</param>
        /// <param name="rge">Précise si la qualification porte la mention RGE (Reconnu Garant de l&#39;Environnement).</param>
        public QualificationsInner(string nom = default(string), string codeQualification = default(string), string definition = default(string), bool rge = default(bool))
        {
            this.Nom = nom;
            this.CodeQualification = codeQualification;
            this.Definition = definition;
            this.Rge = rge;
        }

        /// <summary>
        /// Nom de la qualification
        /// </summary>
        /// <value>Nom de la qualification</value>
        [DataMember(Name = "nom", EmitDefaultValue = false)]
        public string Nom { get; set; }

        /// <summary>
        /// Code de la qualification
        /// </summary>
        /// <value>Code de la qualification</value>
        [DataMember(Name = "code_qualification", EmitDefaultValue = false)]
        public string CodeQualification { get; set; }

        /// <summary>
        /// Définition de la qualification
        /// </summary>
        /// <value>Définition de la qualification</value>
        [DataMember(Name = "definition", EmitDefaultValue = false)]
        public string Definition { get; set; }

        /// <summary>
        /// Précise si la qualification porte la mention RGE (Reconnu Garant de l&#39;Environnement)
        /// </summary>
        /// <value>Précise si la qualification porte la mention RGE (Reconnu Garant de l&#39;Environnement)</value>
        [DataMember(Name = "rge", EmitDefaultValue = true)]
        public bool Rge { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class QualificationsInner {\n");
            sb.Append("  Nom: ").Append(Nom).Append("\n");
            sb.Append("  CodeQualification: ").Append(CodeQualification).Append("\n");
            sb.Append("  Definition: ").Append(Definition).Append("\n");
            sb.Append("  Rge: ").Append(Rge).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as QualificationsInner);
        }

        /// <summary>
        /// Returns true if QualificationsInner instances are equal
        /// </summary>
        /// <param name="input">Instance of QualificationsInner to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(QualificationsInner input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.Nom == input.Nom ||
                    (this.Nom != null &&
                    this.Nom.Equals(input.Nom))
                ) && 
                (
                    this.CodeQualification == input.CodeQualification ||
                    (this.CodeQualification != null &&
                    this.CodeQualification.Equals(input.CodeQualification))
                ) && 
                (
                    this.Definition == input.Definition ||
                    (this.Definition != null &&
                    this.Definition.Equals(input.Definition))
                ) && 
                (
                    this.Rge == input.Rge ||
                    this.Rge.Equals(input.Rge)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.Nom != null)
                {
                    hashCode = (hashCode * 59) + this.Nom.GetHashCode();
                }
                if (this.CodeQualification != null)
                {
                    hashCode = (hashCode * 59) + this.CodeQualification.GetHashCode();
                }
                if (this.Definition != null)
                {
                    hashCode = (hashCode * 59) + this.Definition.GetHashCode();
                }
                hashCode = (hashCode * 59) + this.Rge.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
