/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData
    /// </summary>
    [DataContract(Name = "_v3_cma_france_rnm_unites_legales__siren__get_200_response_data")]
    public partial class V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData : IEquatable<V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData>, IValidatableObject
    {
        /// <summary>
        /// Détermine si l&#39;entreprise a une activité permanente ou saisonnière.   L&#39;activité est dite saisonnière si chaque année, l&#39;entreprise cesse totalement ses activités pendant plus de 3 mois consécutifs.     Les valeurs possibles sont:   - P &#x3D;&gt; Permanent   - S &#x3D;&gt; Saisonnière   - NR &#x3D;&gt; Non renseignée
        /// </summary>
        /// <value>Détermine si l&#39;entreprise a une activité permanente ou saisonnière.   L&#39;activité est dite saisonnière si chaque année, l&#39;entreprise cesse totalement ses activités pendant plus de 3 mois consécutifs.     Les valeurs possibles sont:   - P &#x3D;&gt; Permanent   - S &#x3D;&gt; Saisonnière   - NR &#x3D;&gt; Non renseignée</value>
        [JsonConverter(typeof(StringEnumConverter))]
        public enum ModaliteExerciceEnum
        {
            /// <summary>
            /// Enum P for value: P
            /// </summary>
            [EnumMember(Value = "P")]
            P = 1,

            /// <summary>
            /// Enum S for value: S
            /// </summary>
            [EnumMember(Value = "S")]
            S = 2,

            /// <summary>
            /// Enum NR for value: NR
            /// </summary>
            [EnumMember(Value = "NR")]
            NR = 3

        }


        /// <summary>
        /// Détermine si l&#39;entreprise a une activité permanente ou saisonnière.   L&#39;activité est dite saisonnière si chaque année, l&#39;entreprise cesse totalement ses activités pendant plus de 3 mois consécutifs.     Les valeurs possibles sont:   - P &#x3D;&gt; Permanent   - S &#x3D;&gt; Saisonnière   - NR &#x3D;&gt; Non renseignée
        /// </summary>
        /// <value>Détermine si l&#39;entreprise a une activité permanente ou saisonnière.   L&#39;activité est dite saisonnière si chaque année, l&#39;entreprise cesse totalement ses activités pendant plus de 3 mois consécutifs.     Les valeurs possibles sont:   - P &#x3D;&gt; Permanent   - S &#x3D;&gt; Saisonnière   - NR &#x3D;&gt; Non renseignée</value>
        [DataMember(Name = "modalite_exercice", IsRequired = true, EmitDefaultValue = true)]
        public ModaliteExerciceEnum ModaliteExercice { get; set; }
        /// <summary>
        /// Indique si l&#39;entreprise a une activité ambulante.     Les valeurs possibles sont:   - 0 &#x3D;&gt; sédentaire   - 1 &#x3D;&gt; non-sédentaire
        /// </summary>
        /// <value>Indique si l&#39;entreprise a une activité ambulante.     Les valeurs possibles sont:   - 0 &#x3D;&gt; sédentaire   - 1 &#x3D;&gt; non-sédentaire</value>
        [JsonConverter(typeof(StringEnumConverter))]
        public enum NonSedentaireEnum
        {
            /// <summary>
            /// Enum _0 for value: 0
            /// </summary>
            [EnumMember(Value = "0")]
            _0 = 1,

            /// <summary>
            /// Enum _1 for value: 1
            /// </summary>
            [EnumMember(Value = "1")]
            _1 = 2

        }


        /// <summary>
        /// Indique si l&#39;entreprise a une activité ambulante.     Les valeurs possibles sont:   - 0 &#x3D;&gt; sédentaire   - 1 &#x3D;&gt; non-sédentaire
        /// </summary>
        /// <value>Indique si l&#39;entreprise a une activité ambulante.     Les valeurs possibles sont:   - 0 &#x3D;&gt; sédentaire   - 1 &#x3D;&gt; non-sédentaire</value>
        [DataMember(Name = "non_sedentaire", IsRequired = true, EmitDefaultValue = true)]
        public NonSedentaireEnum NonSedentaire { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData" /> class.
        /// </summary>
        /// <param name="modaliteExercice">Détermine si l&#39;entreprise a une activité permanente ou saisonnière.   L&#39;activité est dite saisonnière si chaque année, l&#39;entreprise cesse totalement ses activités pendant plus de 3 mois consécutifs.     Les valeurs possibles sont:   - P &#x3D;&gt; Permanent   - S &#x3D;&gt; Saisonnière   - NR &#x3D;&gt; Non renseignée (required).</param>
        /// <param name="nonSedentaire">Indique si l&#39;entreprise a une activité ambulante.     Les valeurs possibles sont:   - 0 &#x3D;&gt; sédentaire   - 1 &#x3D;&gt; non-sédentaire (required).</param>
        public V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData(ModaliteExerciceEnum modaliteExercice = default(ModaliteExerciceEnum), NonSedentaireEnum nonSedentaire = default(NonSedentaireEnum))
        {
            this.ModaliteExercice = modaliteExercice;
            this.NonSedentaire = nonSedentaire;
        }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData {\n");
            sb.Append("  ModaliteExercice: ").Append(ModaliteExercice).Append("\n");
            sb.Append("  NonSedentaire: ").Append(NonSedentaire).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData);
        }

        /// <summary>
        /// Returns true if V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData instances are equal
        /// </summary>
        /// <param name="input">Instance of V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(V3CmaFranceRnmUnitesLegalesSirenGet200ResponseData input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.ModaliteExercice == input.ModaliteExercice ||
                    this.ModaliteExercice.Equals(input.ModaliteExercice)
                ) && 
                (
                    this.NonSedentaire == input.NonSedentaire ||
                    this.NonSedentaire.Equals(input.NonSedentaire)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                hashCode = (hashCode * 59) + this.ModaliteExercice.GetHashCode();
                hashCode = (hashCode * 59) + this.NonSedentaire.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
