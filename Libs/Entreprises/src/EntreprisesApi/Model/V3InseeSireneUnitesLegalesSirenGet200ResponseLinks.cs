/*
 * API Entreprise
 *
 * Cette page contient la documentation technique pour accéder à API Entreprise. Les API étant accessible uniquement sous habilitation, l'interaction avec l'environnement de production n'est possible que si vous êtes **en possession d'une clé d'accès (jeton).  ### Comment tester l'API ?  Il est possible de tester les API via notre environnement de **staging** qui vous retournera systématiquement des données fictives.  Il est nécessaire d'utiliser le jeton de staging indiqué ci-dessous.  - --  *eyJhbGciOiJIUzI1NiJ9..u2kMWzll3iCTczUOqMQbpS66VfrVzI2lLiyGEPcKAec*       
 *
 * The version of the OpenAPI document: 3.0.0
 * Contact: support@entreprise.api.gouv.fr
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = EntreprisesApi.Client.OpenAPIDateConverter;

namespace EntreprisesApi.Model
{
    /// <summary>
    /// V3InseeSireneUnitesLegalesSirenGet200ResponseLinks
    /// </summary>
    [DataContract(Name = "_v3_insee_sirene_unites_legales__siren__get_200_response_links")]
    public partial class V3InseeSireneUnitesLegalesSirenGet200ResponseLinks : IEquatable<V3InseeSireneUnitesLegalesSirenGet200ResponseLinks>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="V3InseeSireneUnitesLegalesSirenGet200ResponseLinks" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected V3InseeSireneUnitesLegalesSirenGet200ResponseLinks() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="V3InseeSireneUnitesLegalesSirenGet200ResponseLinks" /> class.
        /// </summary>
        /// <param name="siegeSocial">Lien vers la ressource &#39;Données de référence d&#39;un établissement&#39; correspondant au siège social de cette unité légale. (required).</param>
        /// <param name="siegeSocialAdresse">Lien vers la ressource &#39;Adresse d&#39;un établissement&#39; correspondant au siège social de cette unité légale. (required).</param>
        public V3InseeSireneUnitesLegalesSirenGet200ResponseLinks(string siegeSocial = default(string), string siegeSocialAdresse = default(string))
        {
            // to ensure "siegeSocial" is required (not null)
            if (siegeSocial == null)
            {
                throw new ArgumentNullException("siegeSocial is a required property for V3InseeSireneUnitesLegalesSirenGet200ResponseLinks and cannot be null");
            }
            this.SiegeSocial = siegeSocial;
            // to ensure "siegeSocialAdresse" is required (not null)
            if (siegeSocialAdresse == null)
            {
                throw new ArgumentNullException("siegeSocialAdresse is a required property for V3InseeSireneUnitesLegalesSirenGet200ResponseLinks and cannot be null");
            }
            this.SiegeSocialAdresse = siegeSocialAdresse;
        }

        /// <summary>
        /// Lien vers la ressource &#39;Données de référence d&#39;un établissement&#39; correspondant au siège social de cette unité légale.
        /// </summary>
        /// <value>Lien vers la ressource &#39;Données de référence d&#39;un établissement&#39; correspondant au siège social de cette unité légale.</value>
        [DataMember(Name = "siege_social", IsRequired = true, EmitDefaultValue = true)]
        public string SiegeSocial { get; set; }

        /// <summary>
        /// Lien vers la ressource &#39;Adresse d&#39;un établissement&#39; correspondant au siège social de cette unité légale.
        /// </summary>
        /// <value>Lien vers la ressource &#39;Adresse d&#39;un établissement&#39; correspondant au siège social de cette unité légale.</value>
        [DataMember(Name = "siege_social_adresse", IsRequired = true, EmitDefaultValue = true)]
        public string SiegeSocialAdresse { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class V3InseeSireneUnitesLegalesSirenGet200ResponseLinks {\n");
            sb.Append("  SiegeSocial: ").Append(SiegeSocial).Append("\n");
            sb.Append("  SiegeSocialAdresse: ").Append(SiegeSocialAdresse).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as V3InseeSireneUnitesLegalesSirenGet200ResponseLinks);
        }

        /// <summary>
        /// Returns true if V3InseeSireneUnitesLegalesSirenGet200ResponseLinks instances are equal
        /// </summary>
        /// <param name="input">Instance of V3InseeSireneUnitesLegalesSirenGet200ResponseLinks to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(V3InseeSireneUnitesLegalesSirenGet200ResponseLinks input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.SiegeSocial == input.SiegeSocial ||
                    (this.SiegeSocial != null &&
                    this.SiegeSocial.Equals(input.SiegeSocial))
                ) && 
                (
                    this.SiegeSocialAdresse == input.SiegeSocialAdresse ||
                    (this.SiegeSocialAdresse != null &&
                    this.SiegeSocialAdresse.Equals(input.SiegeSocialAdresse))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.SiegeSocial != null)
                {
                    hashCode = (hashCode * 59) + this.SiegeSocial.GetHashCode();
                }
                if (this.SiegeSocialAdresse != null)
                {
                    hashCode = (hashCode * 59) + this.SiegeSocialAdresse.GetHashCode();
                }
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
