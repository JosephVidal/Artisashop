...
public static class AccountControllerTest
{
    ...
    [Order(1)]
    [TestCase("jean.epp@epitech.eu", "Jean", "Epp", "Password_1234", Roles.Admin, null, TestName = "Add Jean Epp",
        Category = "Register Success")]
    [TestCase("john.artisan@epitech.eu", "John", "Artisan", "Password_1234", Roles.Seller, "Ebeniste",
        TestName = "Add John Artisan", Category = "Register Success")]
    [TestCase("baby.partner@epitech.eu", "Baby", "Partner", "Password_1234", Roles.Partner, null,
        TestName = "Add Baby Partner", Category = "Register Success")]
    [TestCase("jane.consumer@epitech.eu", "Jane", "Consumer", "Password_1234", Roles.User, null,
        TestName = "Add Jane Consumer", Category = "Register Success")]
    [TestCase("delete.user@epitech.eu", "Delete", "Account", "Password_1234", Roles.User, null,
        TestName = "Add Delete Account", Category = "Register Success")]
    [TestCase("delete.user@epitech.eu", "Delete", "Account", "Password_1234", Roles.User, null,
        "Error: DuplicateUserName - Username 'delete.user@epitech.eu' is already taken.",
        TestName = "Account already exist", Category = "Register Fail")]
    public static async Task Register(string email, string firstname, string lastname, string password, string role,
        string job, string expt = null)
    {
        Register model = new()
        {
            Email = email,
    ...
    [Order(2)]
    [TestCase("delete.user@epitech.eu", "Password_1234", TestName = "Login Delete Account",
        Category = "Login Success")]
    [TestCase("delete.user@epitech.eu", "", "Failed", typeof(SignInResult), TestName = "Login Fail",
        Category = "Login Fail")]
    ...
    [TestCase("rugfqoelhfpfdbqoezgf", "Craftsman with id rugfqoelhfpfdbqoezgf not found",
        TestName = "GetAccountId Fail", Category = "GetAccountId Fail")]
    public static async Task GetAccountId(string id, string expt)
    {
        var postRequest = new HttpRequestMessage(HttpMethod.Get,
            "api/account/" + (int.TryParse(id, out int idt) == true ? ids[idt] : id));
        postRequest.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ...
    [Order(3)]
    [Test]
    public static async Task ExternalLogin()
    {
        var basePath = "https://localhost:7095";
        AccountApi api = new AccountApi(basePath);

        var provider = FranceConnectConfiguration.ProviderScheme;

        api.ApiAccountExternalLoginPostWithHttpInfo(provider, "https://example.com");
    }
