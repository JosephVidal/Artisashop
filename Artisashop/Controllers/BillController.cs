    ...
    public class BillController : ControllerBase
    {
        ...
        public async Task<IActionResult> BasketToPaypalBill()
        {
            try
            {
                ...
                List<Basket> basket = await _db.Baskets!.Where(basket => basket.AccountId == account.Id && (basket.CurrentState == State.WAITINGCONSUMER || basket.CurrentState == State.WAITINGCRAFTSMAN)).ToListAsync();
        ...
        public async Task<IActionResult> BasketSold()
        {
            try
            {
                ...
                List<Basket> basket = await _db.Baskets!.Include("Product.Craftsman").Where(basket => basket.AccountId == account.Id && basket.CurrentState == State.WAITINGCONSUMER).ToListAsync();
