    ...
    public class CustomOrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(CreateCustomOrder order)
        {
            try
            {
                ...
                Basket basket = new(account, product, order.Quantity, Basket.DeliveryOption.DELIVERY, Basket.State.WAITINGCRAFTSMAN, GenPossibleState(Basket.State.WAITINGCRAFTSMAN, Basket.DeliveryOption.DELIVERY));
        ...
        public async Task<IActionResult> Update([FromBody] UpdateCustomOrder model)
        {
            try
            {
                ...
                basket.DeliveryOpt = model.DeliveryOpt == null ? basket.DeliveryOpt : (Basket.DeliveryOption)model.DeliveryOpt;
        ...
        [ProducesResponseType(typeof(string), (int)HttpStatusCode.BadRequest)]
        [ProducesResponseType(typeof(Basket.State), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> UpdateStatus(int basketId, Basket.State state)
        {
            try
            {
        ...
        private static List<Basket.State> GenPossibleState(Basket.State state, Basket.DeliveryOption delOpt)
        {
            switch (state)
            {
                case Basket.State.WAITINGCRAFTSMAN:
                    return new List<Basket.State> { Basket.State.REFUSED/*, State.WAITINGCONSUMER*/ };
                case Basket.State.VALIDATED:
                    if (Basket.DeliveryOption.TAKEOUT == delOpt)
                        return new List<Basket.State> { Basket.State.ONGOING, Basket.State.DELIVERY, Basket.State.END };
                    else
                        return new List<Basket.State> { Basket.State.ONGOING, Basket.State.END };
                case Basket.State.ONGOING:
                    if (Basket.DeliveryOption.TAKEOUT == delOpt)
                        return new List<Basket.State> { Basket.State.DELIVERY, Basket.State.END };
                    else
                        return new List<Basket.State> { Basket.State.END };
                case Basket.State.DELIVERY:
                    return new List<Basket.State> { Basket.State.END };
                case Basket.State.WAITINGCONSUMER:
                case Basket.State.REFUSED:
                case Basket.State.END:
                default:
                    return new List<Basket.State>();
            }
        }
    }
}
