...
namespace Artisashop.Controllers
{
    ...
    public class BackofficeController : ControllerBase
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                List<Account> list = await _db.Accounts!.Where(user => user.Role == Account.UserType.CRAFTSMAN).ToListAsync();
                return Ok(list);
            }
            catch (Exception e)
            {
        ...
        public async Task<IActionResult> Stats()
        {
            try
            {
                Home viewModel = new()
                {
                    CraftsmanNumber = (await _db.Accounts!.Where(account => account.Role == Account.UserType.CRAFTSMAN).ToListAsync()).Count,
                    ProductNumber = (await _db.Products!.ToListAsync()).Count,
                    Inscrit = (await _db.Accounts!.Where(account => account.Role == Account.UserType.CONSUMER || account.Role == Account.UserType.CRAFTSMAN).ToListAsync()).Count
                };
                return Ok(viewModel);
            }
            catch (Exception e)
