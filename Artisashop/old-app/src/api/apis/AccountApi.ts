...
export class AccountApi extends runtime.BaseAPI {
    ...
    async apiAccountIdGetRaw(requestParameters: ApiAccountIdGetRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<Account>> {
        if (requestParameters.id2 === null || requestParameters.id2 === undefined) {
            throw new runtime.RequiredError('id2','Required parameter requestParameters.id2 was null or undefined when calling apiAccountIdGet.');
        }
        ...
        const response = await this.request({
            path: `/api/account/{id}`.replace(`{${"id"}}`, encodeURIComponent(String(requestParameters.id2))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);
